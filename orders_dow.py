...
def insert_order(connection, order):
    ...
    order_query = ("INSERT INTO orders "
             "(coustomer_name, total, datetime)"
             "VALUES (%s, %s, %s)")
    order_data = (order['coustomer_name'], order['grand_total'], datetime.now())

    cursor.execute(order_query, order_data)
    order_id = cursor.lastrowid

    order_details_query = ("INSERT INTO order_table "
                           "(order_id, product_id, quantity, total_price)"
                           "VALUES (%s, %s, %s, %s)")

    order_details_data = []
    for order_detail_record in order['order_table']:
